...
static const std::map<std::set<char>, int> reference = {
    {{'c', 'f'}, 1},
    {{'a', 'b', 'c', 'e', 'f', 'g'}, 0},
    {{'a', 'c', 'd', 'e', 'g'}, 2},
    {{'a', 'c', 'd', 'f', 'g'}, 3},
...
void split(const std::string &row,
           std::vector<std::vector<std::string>> &array_patterns,
           std::vector<std::vector<std::string>> &array_outputs) {
  std::string symbol;
...
int calculate_first(
    const std::vector<std::vector<std::string>> &input_outputs) {
  int sum = 0;
...
std::set<char> to_set(const std::string &str,
...
bool map_pattern(const std::vector<std::string> &input_patterns,
                 std::unordered_map<char, char> &wire,
                 const std::unordered_set<char> &vars, int str_ind,
                 int char_ind) {
  if (char_ind >= input_patterns[str_ind].size()) {
    if (reference.find(to_set(input_patterns[str_ind], wire)) !=
        reference.end()) {
      str_ind++;
      char_ind = 0;
    } else
...
bool compare(std::string &s1, std::string &s2) { return s1.size() < s2.size(); }

std::unordered_map<std::string, int>
update_map(const std::vector<std::string> &input_patterns,
           std::unordered_map<char, char> &wire) {
  ...
  for (const auto &s : input_patterns) {
    r_m.insert({s, reference.find(to_set(s, wire))->second});
  }
  return r_m;
}

int calculate_second(
    const std::vector<std::vector<std::string>> &input_patterns,
    const std::vector<std::vector<std::string>> &input_outputs) {
  int sum = 0;
...
void parse_input(const std::string &fileName) {
  ...
  if (input_outputs.empty() || input_patterns.empty()) {
    std::cout << "Empty input file \n";
  }

  std::cout << calculate_first(input_outputs) << "-"
            << calculate_second(input_patterns, input_outputs) << "\n";
}

int main(int argc, char *argv[]) { parse_input("input.txt"); }
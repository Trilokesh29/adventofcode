...
std::uint32_t GetCount_Yes_From_Everyone_In_A_Group(
    const std::vector<std::string> &aGroupInfo) {
  if (1 == aGroupInfo.size()) {
    return aGroupInfo.at(0).size();
  }

  std::map<char, int> yesToCountMap;
}
...
uint32_t
second_solution(const std::vector<std::vector<std::string>> &inputData) {
  uint32_t count = 0;
...
int main() {
  ...
  std::cout << "first solution = " << CalculateSum(inputData);
  std::cout << "second solution = " << second_solution(inputData);
